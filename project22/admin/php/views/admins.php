...
<div class="jq_contents" id="admin">
	<div class="container-fluid ">
		<div class="row br-bt-blu">
			<div class="col-sm-3">
				<h3 class="mg-0">&nbsp;&nbsp;&nbsp;<big><i class="fa fa-user"></i></big> Admins</h3>
			</div>
			<div class="col-sm-6">
		...
		<div class="container-fluid" id="alluser">
			<?php
				...
				while($row=mysqli_fetch_array($res)){
					...
					echo ("<tr>");
					echo ("<td><center>Name :<input type='text' value='{$row["first_name"]}' readonly  class='text-center txt-wh form-control name bd-tp-bd-0'></center></td>");
				 	echo ("<td><center>Email :<input type='text' value='{$row["email"]}' readonly class='text-center form-control txt-wh email bd-tp-bd-0' ></center></td>");
					echo ("<td><center>Username:<input type='text' value='{$row["uid"]}' readonly class='text-center  username form-control txt-wh bd-tp-bd-0'></center></td>");
					echo ("<td><center>Password : <input type='text' value='{$row["pwd"]}' readonly class='text-center form-control txt-wh password bd-tp-bd-0'></center></td>");
				    
					echo ("<td rowspan='2'>
					<center><button class='btn btn-link cancel_account_table txt-wh disp-none'><i class='fa fa-edit txt-wh'></i> Cancel</button></center>
					<center><button class='btn btn-link disable_account_btn txt-wh' disable_id='$userid' deleteid='$deleteid'=><i class='fa fa-trash txt-wh'></i> Delete</button></center><br />
					...
					echo ("<tr>");
					echo ("<td>Scope: <span id ='scope'>{$per[$row["scope"]]}</span>
					<select class='disp-none text-primary scope'>
					<option value='0'>None</option>
					<option value='1'>add</option>
					<option value='2'>delete</option>
					<option value='3'>add/delete</option></select>
					</td>");
				 	echo ("<td>Category:<span id ='category'>{$per[$row["category"]]}</span>
					<select class='disp-none text-primary category'>
					<option value='0'>None</option>
					<option value='1'>add</option>
					<option value='2'>delete</option>
					<option value='3'>add/delete</option></select>
					</td>");
					echo ("<td>Topic :<span id='topic'> {$per[$row["topic"]]}</span>
					
					<select class='disp-none text-primary topic'>
					<option value='0'>None</option>
					<option value='1'>add</option>
					<option value='2'>delete</option>
					<option value='3'>add/delete</option></select></td>");
					echo ("<td>Ebooks :<span id='tutorial'> {$per[$row["ebooks"]]}</span>
					
					<select class='disp-none text-primary tutorial'>
					<option value='0'>None</option>
					<option value='1'>add</option>
					<option value='2'>delete</option>
